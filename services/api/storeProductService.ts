...
export class StoreProductService {
  ...
  async getGroceryProductsBySubcategoryPaged(
    ...
    try {
      ...
      const response = await apiClient.get<any>(url, { limit, page });

      const raw = response.data;
      const rawProducts = Array.isArray(raw?.data) ? raw.data : [];

      // Filter products matching this subcategoryId using multiple possible fields/shapes
      const filteredRaw = rawProducts.filter((p: any) => {
        // Nested subcategory object
        if (
          p.subcategories?.some(
            (sub: any) => sub.subcategoryId === subcategoryId,
          )
        ) {
          return true;
        }

        // Subcategories array (if backend sends an array)
        if (Array.isArray(p.subcategories)) {
          const hasMatchingSubcategory = p.subcategories.some(
            (sub: any) =>
  ...
  async getPharmaProductsBySubcategoryPaged(
    ...
    try {
      ...
      // Backend does not support subcategory filter here; fetch a page and filter on client
      const response = await apiClient.get<any>(url, { limit, page });
